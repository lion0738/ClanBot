...
public class InitNotifyCommand implements ICommand {
	...
	public SlashCommandData getCommandData() {
		return Commands.slash(COMMAND, "Enable clan war notifications in this channel.")
				.addOption(OptionType.STRING, TAG_OPTION, "Tag of the clan (ex. TECH)", true);
	}
	...
	public void onSlashCommand(InteractionHook hook, List<OptionMapping> options, Callback callback) {
		...
		if (clanTag != null) {
			if (ClanWarNotifier.addServer(hook.getInteraction().getChannelId(), clanTag)) {
				hook.editOriginal("Server notifications have been set. "
						+ "You will receive notifications for upcoming clan war alerts.").queue();
			} else {
				hook.editOriginal("Server notifications are already set for the specified clan tag.").queue();
			}
		} else {
